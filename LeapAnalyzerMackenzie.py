...
class LeapAnalyzerMackenzie:
    ...
    def calculateMovementDirectionChange(self):
        ...
        # decide the initial direction
        prevDirectionX = self.judgeDirectionX(float(prevFrame[colNumSplitX]),float(currentFrame[colNumSplitX]))
        prevDirectionY = self.judgeDirectionY(float(prevFrame[colNumSplitY]),float(currentFrame[colNumSplitY]))
        prevDirectionZ = self.judgeDirectionZ(float(prevFrame[colNumSplitZ]),float(currentFrame[colNumSplitZ]))

        i = 2  # started from the third frame
        prevFrame=self.frameArray[1] # let frame[2] be the previous frame
        while i < len(self.frameArray):
            ...
            # if not the same direction, self.movementDirectionChangX++
            currentDirectionX = self.calculateMovementDirectionChangeX(prevX, currentX, prevDirectionX)
            # update the self.movementDirectionChangY and return the current direction of Y
            currentDirectionY = self.calculateMovementDirectionChangeY(prevY, currentY, prevDirectionY)
            # update the self.movementDirectionChangZ and return the current direction of Z
            currentDirectionZ = self.calculateMovementDirectionChangeZ(prevZ, currentZ, prevDirectionZ)
    ...
    def judgeDirectionX(self,prevX,currentX):
        if prevX != currentX:
            if currentX < prevX:
    ...
    def calculateMovementDirectionChangeX(self,prevX, currentX, prevDirectionX):
        currentDirectionX=self.judgeDirectionX(prevX,currentX) # get the current direction
        if prevDirectionX != currentDirectionX:
            self.movementDirectionChangeX=self.movementDirectionChangeX+1
        return currentDirectionX


    def judgeDirectionY(self,prevY,currentY):
        if prevY != currentY:
            if currentY <prevY:
    ...
    def calculateMovementDirectionChangeY(self,prevY,currentY,prevDirectionY):
        currentDirectionY=self.judgeDirectionY(prevY,currentY)
        if prevDirectionY!=currentDirectionY:
            self.movementDirectionChangeY=self.movementDirectionChangeY+1
        return currentDirectionY

    def judgeDirectionZ(self,prevZ,currentZ):
        if prevZ != currentZ:
            if currentZ < prevZ:
    ...
    def calculateMovementDirectionChangeZ(self,prevZ,currentZ,prevDirectionZ):
        currentDirectionZ=self.judgeDirectionZ(prevZ,currentZ)
    ...
    def calculateMovementVariability(self,movementOffset):
        ...
        for i in range(0,len(self.frameArray)):
            ...
            differenceMovementErrorSqr=math.pow(self.calculateRealMovementError(self.startX,self.startY,self.startZ,
            self.targetX,self.targetY,self.targetZ,currentX,currentY,currentZ)-movementOffset,2)
        ...
        # MV is sample standard deviation rather than standard deviation so we need to divide by length-1
        movementVariability = math.sqrt(sumMovementErrorDifference / (len(self.frameArray) - 1.0))
    ...
    def calculateMovementOffset(self):
        ...
        for i in range(0, len(self.frameArray)):
            ...
            currentY = float(currentFrame[colNumSplitY])
            currentZ = float(currentFrame[colNumSplitZ])
            sumMovementError = sumMovementError + self.calculateRealMovementError(self.startX,
            self.startY, self.startZ,self.targetX, self.targetY,self.targetZ, currentX, currentY,currentZ)
    ...
    # task plane crossing happens when the path of the finger passes through the the task plane
    # the task plane means a plane vertical to the tablet plane
    # the task plane and the tablet plane intersects at the task axis
    def calculateTaskPlaneCrossing(self):

        startFrame=self.frameArray[0]
        # judge the initial point is above or below the task plane
        if self.judgeAboveOrBelowPlane(float(startFrame[colNumSplitX]),float(startFrame[colNumSplitY]),float(startFrame[colNumSplitZ]))==True:# true means above
            preAbove=True
        else:
            preAbove=False
